import { createRouter, createWebHashHistory } from 'vue-router'
import { isAuthenticated } from './api'

import Login from './views/Login.vue'
import Dashboard from './views/Dashboard.vue'
import Brokers from './views/Brokers.vue'
import Tools from './views/Tools.vue'
import Backtest from './views/Backtest.vue'
import Strategies from './views/Strategies.vue'
import LiveTrade from './views/LiveTrade.vue'
import ImportData from './views/ImportData.vue'
import LLMStudio from './views/LLMStudio.vue'
import Settings from './views/Settings.vue'
import Optimization from './views/Optimization.vue'
import MonteCarlo from './views/MonteCarlo.vue'
import Issues from './views/Issues.vue'

const routes = [
  { path: '/login', name: 'Login', component: Login, meta: { public: true } },
  { path: '/', name: 'Dashboard', component: Dashboard },
  { path: '/brokers', name: 'Brokers', component: Brokers },
  { path: '/tools', name: 'Tools', component: Tools },
  { path: '/instruments', redirect: '/tools' },
  { path: '/strategies', name: 'Strategies', component: Strategies },
  { path: '/backtest', name: 'Backtest', component: Backtest },
  { path: '/optimization', name: 'Optimization', component: Optimization },
  { path: '/monte-carlo', name: 'Monte Carlo', component: MonteCarlo },
  { path: '/live', name: 'Live Trade', component: LiveTrade },
  { path: '/import', name: 'Import Data', component: ImportData },
  { path: '/llm', name: 'LLM Studio', component: LLMStudio },
  { path: '/settings', name: 'Settings', component: Settings },
  { path: '/issues', name: 'Issues', component: Issues },
]

const router = createRouter({
  history: createWebHashHistory(),
  routes,
})

router.beforeEach((to) => {
  if (!to.meta.public && !isAuthenticated()) {
    return { name: 'Login' }
  }
})

export default router
