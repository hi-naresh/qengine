...
import { logout, hasFeature, isAdmin } from '../api'
...
const tradingPaths = ['/strategies', '/backtest', '/optimization', '/monte-carlo', '/autopilot', '/live', '/import']
const morePaths = ['/tools', '/llm', '/issues', '/help', '/admin', '/settings']
...
const allTradingItems = [
  { to: '/strategies', label: 'Strategies', icon: StratIcon, feature: 'strategies' },
...
const allMoreItems = [
  { to: '/tools', label: 'Tools', icon: InstrIcon, feature: 'tools' },
  { to: '/llm', label: 'LLM Studio', icon: LLMIcon, feature: 'llm_studio' },
  { to: '/issues', label: 'Issues', icon: IssuesIcon, feature: 'issues' },
...
const moreItems = computed(() => allMoreItems.filter(i => {
  if (i.adminOnly) return isAdmin()
  return !i.feature || hasFeature(i.feature)
}))
</script>
