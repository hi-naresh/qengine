...
import traceback
from qengine.services.redis import sync_publish, sync_redis
from qengine.services.failure import terminate_session
...
class ProcessManager:
    ...
    def add_task(self, function, *args):
        ...
        self.client_id_to_pid_map[self._prefixed_client_id(client_id)] = self._prefixed_pid(w.pid)
    ...
    def get_pid(self, client_id):
        return self.client_id_to_pid_map[self._prefixed_client_id(client_id)]

    def cancel_process(self, client_id):
        sync_redis.srem(self._active_workers_key, client_id)

        # Give the process up to 10s to exit gracefully (run _graceful_shutdown),
        # then force-terminate if it's still alive.
        def _force_kill_after_timeout():
            time.sleep(10)
            prefixed = self._prefixed_client_id(client_id)
            pid_str = self.client_id_to_pid_map.get(prefixed)
            if not pid_str:
                return
            pid = int(pid_str.split('|')[-1])
            for w in self._workers:
                if w.pid == pid and w.is_alive():
                    jh.debug(f"Force-terminating process {client_id} (pid={pid}) after graceful timeout")
                    try:
                        w.terminate()
                        w.join(timeout=3)
                        if w.is_alive():
