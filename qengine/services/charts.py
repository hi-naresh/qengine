...
from qengine.services.candle_service import get_candles_from_db
from qengine.utils import prices_to_returns
    ...
    if benchmark:
        ...
        for i, r in enumerate(router.routes):
            _, daily_candles = get_candles_from_db(
                ...
                store.app.ending_time + 1000 * 60 * 60 * 24, is_for_engine=False, warmup_candles_num=0, caching=True
            )
...
def floating_pnl_curve() -> dict:
    daily_pnl = store.app.daily_floating_pnl
    if not daily_pnl or len(daily_pnl) < 2:
        return None
    start_date = datetime.fromtimestamp(store.app.starting_time / 1000)
    return _calculate_equity_curve(daily_pnl, start_date, 'Floating PnL', '#fbbf24')


def margin_usage_curve() -> dict:
    daily_margin = store.app.daily_margin_used
    if not daily_margin or len(daily_margin) < 2:
        return None
    start_date = datetime.fromtimestamp(store.app.starting_time / 1000)
    return _calculate_equity_curve(daily_margin, start_date, 'Margin Used', '#fb7185')
