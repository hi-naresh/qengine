...
def install(is_live_plugin_already_installed: bool, strict: bool):
    if is_live_plugin_already_installed:
        from qengine_live.version import __version__
        click.clear()
        print(f'Version "{__version__}" of the live-trade plugin is already installed.')
        if strict:
            txt = '\nIf you meant to update, first delete the existing version by running "pip uninstall qengine_live -y" and then run "qengine install-live" one more time.'
    ...
    if not ENV_VALUES['LICENSE_API_TOKEN']:
        if strict:
            print("No license API token set. Please set the LICENSE_API_TOKEN environment variable to continue. If you don't have one yet, create one at your QEngine admin panel" )
        return
    else:
    ...
    if is_arm and (os_name in ['linux', 'windows']):
        raise NotImplementedError(f'ARM versions of {os_name} are not supported. If you need them, send a request to support@qengine.dev')
    ...
    try:
        response = requests.post(
            QENGINE_API_URL + '/download-release',
