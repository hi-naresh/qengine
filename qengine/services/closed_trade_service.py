...
from qengine.repositories import closed_trade_repository, order_repository
from qengine.store import store
from qengine.enums import sides
import numpy as np
from qengine.models import ClosedTrade, Order, Position
...
def close_trade(position: Position) -> None:
    ...
    if not t.is_open:
        logger.info(
            ...
            " report using the report button so that QEngine's support team can investigate the issue."
        )
        return
