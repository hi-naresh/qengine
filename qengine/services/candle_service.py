...
import numpy as np
import arrow
from qengine.exceptions import CandleNotFoundInDatabase, InvalidDateRange
...
def split_candle(candle: np.ndarray, price: float) -> tuple:
    ...
    if is_bullish(candle) and l < price < o:
        return np.array([
            timestamp, o, price, o, price, v/2
        ]), np.array([
            timestamp, price, c, h, l, v/2
        ])
    elif price == o:
        return candle, candle
    elif is_bearish(candle) and o < price < h:
        return np.array([
            timestamp, o, price, price, o, v/2
        ]), np.array([
            timestamp, price, c, h, l, v/2
        ])
    elif is_bearish(candle) and l < price < c:
        return np.array([
            timestamp, o, price, h, price, v/2
        ]), np.array([
            timestamp, price, c, c, l, v/2
        ])
    elif is_bullish(candle) and c < price < h:
        return np.array([
            timestamp, o, price, price, l, v/2
        ]), np.array([
            timestamp, price, c, h, c, v/2
        ]),
    elif is_bearish(candle) and price == c:
        return np.array([
            timestamp, o, c, h, c, v/2
        ]), np.array([
            timestamp, price, price, price, l, v/2
        ])
    elif is_bullish(candle) and price == c:
        return np.array([
            timestamp, o, c, c, l, v/2
        ]), np.array([
            timestamp, price, price, h, price, v/2
        ])
    elif is_bearish(candle) and price == h:
        return np.array([
            timestamp, o, h, h, o, v/2
        ]), np.array([
            timestamp, h, c, h, l, v/2
        ])
    elif is_bullish(candle) and price == l:
        return np.array([
            timestamp, o, l, o, l, v/2
        ]), np.array([
            timestamp, l, c, h, l, v/2
        ])
    elif is_bearish(candle) and price == l:
        return np.array([
            timestamp, o, l, h, l, v/2
        ]), np.array([
            timestamp, l, c, c, l, v/2
        ])
    elif is_bullish(candle) and price == h:
        return np.array([
            timestamp, o, h, h, l, v/2
        ]), np.array([
            timestamp, h, c, h, c, v/2
        ])
    elif is_bearish(candle) and c < price < o:
        return np.array([
            timestamp, o, price, h, price, v/2
        ]), np.array([
            timestamp, price, c, price, l, v/2
        ])
    elif is_bullish(candle) and o < price < c:
        return np.array([
            timestamp, o, price, price, l, v/2
        ]), np.array([
            timestamp, price, c, h, price, v/2
        ])


def inject_warmup_candles_to_store(candles: np.ndarray, exchange: str, symbol: str) -> None:
    if candles is None or candles.size == 0:
        # No warmup candles available — proceed without warmup (auto-clamped)
        return
...
def get_candles_from_db(
            ...
            if earliest_rows and earliest_rows[0][0] < trading_start_date_timestamp:
                ...
                try:
                    warmup_candles = _get_candles_from_db(
                        exchange, symbol, clamped_start, warmup_finish_timestamp, caching=caching
                    )
                except Exception:
                    warmup_candles = np.zeros((0, 6))
            else:
                # No warmup data available before trading start — proceed with empty warmup
                warmup_candles = np.zeros((0, 6))
    else:
        warmup_candles = None
    ...
    # if timeframe is 1m or is_for_engine is True, return the candles as is because they
    # are already 1m candles which is the accepted format for practicing with QEngine.
    if timeframe == '1m' or is_for_engine:
...
def add_candle(
    ...
    # allow updating of the previous candle.
        ...
        for i in range(1, min(21, len(arr))):
            if arr[-i][0] == candle[0]:
                arr[-i] = candle
                break
    else:
