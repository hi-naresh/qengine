...
def _save_lsp_version(lsp_version: str) -> None:
    ...
    """
    from qengine import QENGINE_DIR
    version_file = os.path.join(QENGINE_DIR, 'lsp', 'VERSION')
...
def install_lsp_server() -> None:
    ...
    target_dir = os.path.join(QENGINE_DIR, 'lsp')
...
def run_lsp_server():
    ...
    from qengine import QENGINE_DIR
    lsp_dir = os.path.join(QENGINE_DIR, 'lsp')
    ...
    if not os.path.exists(lsp_dir):
        raise Exception("LSP directory not found. Please re-install it first by restarting qengine.")
    
    if not os.path.exists(start_script):
        raise Exception("Python Language Server start script not found. Please re-install it first by restarting qengine.")
    ...
    # Get the parent directory of the QEngine framework
    qengine_framework_parent = os.path.dirname(QENGINE_DIR)
        ...
        with open(os.devnull, 'w') as devnull:
            process = subprocess.Popen(
                [
                    ...
                    '--port', str(port),
                    '--bot-root', bot_root,
                    '--qengine-root', qengine_framework_parent
                ],
