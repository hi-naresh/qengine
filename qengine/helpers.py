...
import numpy as np
import base64
from qengine.constants import CANDLE_SOURCE_MAPPING
from qengine.constants import TIMEFRAME_PRIORITY
from qengine.constants import SUPPORTED_COLORS
...
def app_currency() -> str:
    ...
    if exchange_name in exchange_info and 'settlement_currency' in exchange_info[exchange_name]:
        return exchange_info[exchange_name]['settlement_currency']
    if exchange_name in broker_info and 'settlement_currency' in broker_info[exchange_name]:
        return broker_info[exchange_name]['settlement_currency']
...
def estimate_average_price(order_qty: float, order_price: float, current_qty: float,
    ...
    """
    if abs(order_qty) + abs(current_qty) == 0:
        return 0
...
def quote_asset(symbol: str) -> str:
    try:
        ...
        from qengine.exceptions import InvalidRoutes
...
def get_asset_class(symbol: str) -> str:
    ...
    return instrument_registry.get_asset_class(symbol)


def get_pip_size(symbol: str) -> float:
    from qengine.core.instruments import instrument_registry
    return instrument_registry.get_pip_size(symbol)


def get_contract_size(symbol: str) -> float:
    from qengine.core.instruments import instrument_registry
    return instrument_registry.get_contract_size(symbol)
...
def side_to_type(s: str) -> str:
    from qengine.enums import trade_types, sides
...
def dd(item):
    """
    Dump and Die but pretty: used for debugging
    """
    dump(item)
    terminate_app()
...
def dump(*item):
    """
    Dump object in pretty format: used for debugging
    """
    if len(item) == 1:
        item = item[0]
...
def debug(*item):
    """
    Used for debugging. Prints the item in pretty format in both
...
def terminal_debug(*item):
    """
    Used for debugging. Prints the item with timestamp in the 
...
def timeframe_to_one_minutes(timeframe: str) -> int:
    from qengine.utils import timeframe_to_one_minutes
...
def validate_cwd() -> None:
    """
    make sure we're in a QEngine project
    """
    if not is_qengine_project():
        print(
            color(
                'Current directory is not a QEngine project. You must run commands from the root of a QEngine project directory (containing strategies/ and storage/ folders).',
                'red'
            )
        )
        os._exit(1)
...
def has_live_trade_plugin() -> bool:
    # Check for the external qengine_live package (crypto)
    try:
        import qengine_live
        return True
    except ModuleNotFoundError:
        pass
    # Our built-in forex/CFD live drivers count as live trade support
    try:
