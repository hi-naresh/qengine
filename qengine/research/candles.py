...
from qengine import factories
import qengine.helpers as jh
from qengine.services.candle_service import get_candles_from_db as _get_candles
...
def get_candles(
    ...
    if not jh.is_qengine_project():
        raise FileNotFoundError(
            'Invalid directory: ".env" file not found. To use QEngine inside notebooks, create notebooks inside the root of a QEngine project.'
        )

    return _get_candles(exchange, symbol, timeframe, start_date_timestamp, finish_date_timestamp, warmup_candles_num, caching, is_for_engine)
...
def store_candles(candles: np.ndarray, exchange: str, symbol: str) -> None:
    ...
    """
    from qengine.modes.import_candles_mode import store_candles_list as store_candles_from_list
    import qengine.helpers as jh

    # check if .env file exists
    if not jh.is_unit_testing() and not jh.is_qengine_project():
        raise FileNotFoundError(
            'Invalid directory: ".env" file not found. To use QEngine inside notebooks, create notebooks inside the root of a QEngine project.'
        )
