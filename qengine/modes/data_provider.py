...
from qengine.info import live_trading_exchanges, backtesting_exchanges, broker_info
    ...
    return {
        ...
        'brokers': {
            k: {
                'name': v['name'],
                'type': v['type'],
                'asset_classes': v.get('asset_classes', []),
                'fee_model': v.get('fee_model', 'spread'),
                'default_leverage': v.get('default_leverage', 1),
            }
            for k, v in broker_info.items()
        },
        'backtesting_exchanges': backtesting_exchanges,
        'live_trading_exchanges': live_trading_exchanges,
    }
...
def import_api_keys_from_csv(content: str, user_id: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Keep track of existing names to update instead of duplicate
        existing_keys = {k.name: k for k in ExchangeApiKeys.select()}
        ...
        for row in reader:
            ...
            # Skip rows with missing mandatory fields (api_secret is optional)
            if not all([name, exchange, api_key]):
                continue
...
def _sync_imported_keys_to_settings(imported_keys: list, user_id: str = None):
    ...
    for key_info in imported_keys:
        broker_id = key_info['exchange']
        settings['brokers'][broker_id] = {
            'api_key': key_info['api_key'],
            'api_secret': key_info['api_secret'],
            'account_id': key_info['additional_fields'].get('account_id', ''),
