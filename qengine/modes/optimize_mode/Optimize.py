...
from qengine.services.redis import is_process_active
...
class Optimizer:
    def __init__(
        ...
        # Enrich user_config with resolved exchange config for Ray workers
        # (Ray workers run in separate processes without access to the global config)
    ...
    def _process_trial_result(self, result):
        ...
        # Process trial metrics for objective curve (always, even for low-scoring trials)
        ...
        # Periodically save to DB (outside score check so curve data is always saved)
    ...
    def _process_trial_metrics(self, trial_number, training_metrics, testing_metrics):
        ...
        # Add to buffer if at least training metrics exist (testing can be empty for bad trials)
        if training_metrics:
            data_point = {
            ...
            if jh.is_debugging():
                jh.debug(f"Skipped trial {trial_number + 1} - no training metrics")
