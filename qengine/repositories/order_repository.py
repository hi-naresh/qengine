...
from qengine.models import Candle
...
def create(order_data: dict) -> Order:
    ...
    if 'engine_submitted' in order_data:
        d['engine_submitted'] = order_data.get('engine_submitted')
...
def store_or_update(order: Order) -> None:
    ...
    if order.submitted_via:
        d['submitted_via'] = order.submitted_via
    if hasattr(order, 'engine_submitted'):
        d['engine_submitted'] = order.engine_submitted
