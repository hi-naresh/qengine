...
def crossed(series1: np.ndarray, series2: Union[float, int, np.ndarray], direction: str = None,
    ...
    if direction is not None and direction not in ('above', 'below'):
        raise ValueError(f'Invalid direction: {direction}. Must be "above", "below", or None.')
...
def combinations_without_repeat(a: np.ndarray, n: int = 2) -> np.ndarray:
    ...
    return np.array(list(combinations(a, n)))
...
def timeframe_to_one_minutes(timeframe: str) -> int:
    ...
    """
    from qengine.enums import timeframes
    from qengine.exceptions import InvalidTimeframe
