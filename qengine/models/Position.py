...
class Position:
    ...
    def leverage(self) -> Union[int, np.float64]:
        ...
        if self.exchange_type == 'forex_cfd':
    ...
    def mode(self) -> str:
        if self.exchange.type == 'spot':
            return 'spot'
        elif self.exchange.type == 'forex_cfd':
            return 'forex_cfd'
        else:
    ...
    def margin_used(self) -> float:
        if self.is_close:
            return 0
        # qty is already in units (not lots), so notional = qty * price
        # margin = notional * margin_rate (or notional / leverage)
    ...
    def liquidation_price(self) -> Union[float, np.float64]:
        ...
        if self.mode in ['cross', 'spot', 'forex_cfd']:
            return np.nan
