...
class ClosedTrade(peewee.Model):
    ...
    def __init__(self, attributes: dict = None, **kwargs) -> None:
        ...
        self.orders = []
        # strategy-defined metadata (e.g. hedge session info) — not persisted to DB
        self.meta = {}
    ...
    def entry_price(self) -> float:
        ...
        qty_sum = orders[:, 0].sum()
        if qty_sum == 0:
            return np.nan
        return (orders[:, 0] * orders[:, 1]).sum() / qty_sum
