...
class Exchange(ABC):
    def __init__(self, name: str, starting_balance: float, fee_rate: float, exchange_type: str):
        ...
        # check the settlement_currency in exchange_info or broker_info
        ...
        elif self.name in broker_info and 'settlement_currency' in broker_info[self.name]:
            self.settlement_currency = broker_info[self.name]['settlement_currency']
        else:
