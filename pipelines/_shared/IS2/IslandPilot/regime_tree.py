    ...
    def load(path: str) -> "RegimeTree":
        class _Unpickler(pickle.Unpickler):
            def find_class(self, module, name):
                # remap legacy pickle paths from old module layouts
                if 'qengine.framework.components' in module:
                    module = module.replace('qengine.framework.components',
                                            'pipelines._shared.IslandPilot')
                if 'pipelines._shared.components' in module:
                    module = module.replace('pipelines._shared.components',
                                            'pipelines._shared.IslandPilot')
