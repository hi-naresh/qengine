...
class RegimeInferencer:
    ...
    def __init__(self, regime_tree: Any, config: Optional[dict] = None):
        ...
        # Minimum probability for the best leaf to be considered a "known" regime.
        # Justification: with 73 leaves, random uniform assignment gives each
        # leaf ~1/73 ≈ 1.4% probability. A confident single-regime classification
        # assigns 20-40% to the dominant leaf. The threshold of 0.15 (15%)
        # requires at least 10× concentration above random — below this, the
        # feature vector is spread across too many leaves to indicate a clear
        # market state. This follows from the information-theoretic principle
        # that a classification is only meaningful if P(best class) >> 1/N_classes.
        # (Principle adapted from Cover & Thomas 2006, Elements of Information Theory.)
        self.unknown_threshold = config.get("unknown_threshold", 0.15)
        ...
        self._is_known_regime: bool = True   # updated each classify() call
        self._unknown_count: int = 0         # cumulative unknown classifications
    ...
    def classify(
        ...
        # Unknown regime detection: if no leaf holds a meaningful probability
        # concentration, the current market state is outside the training
        # distribution. We record this as instance state (not in the return
        # value, to avoid breaking callers) and the pipeline should check
        # self.inferencer.is_known_regime before allowing entry.
        best_prob = max(all_probs.values()) if isinstance(all_probs, dict) and all_probs else new_confidence
        self._is_known_regime = best_prob >= self.unknown_threshold
        if not self._is_known_regime:
    ...
    @property
    def is_known_regime(self) -> bool:
        """True if the most recent classification had a confident dominant leaf.

        Returns False when the market state is spread evenly across many regimes
        (feature vector far from all training cluster centers), indicating an
        out-of-distribution market condition. Callers should block entry when
        this is False.
        """
        return self._is_known_regime
    ...
    def get_unknown_stats(self) -> dict:
        """Return stats about unknown-regime classifications."""
        return {
            "unknown_threshold": self.unknown_threshold,
            "unknown_count": self._unknown_count,
            "classify_count": self._classify_count,
            "unknown_rate": (self._unknown_count / self._classify_count
                             if self._classify_count > 0 else 0.0),
            "is_known_regime": self._is_known_regime,
        }
    ...
    def load(cls, path: str, regime_tree: Any) -> "RegimeInferencer":
        ...
        obj.unknown_threshold = data.get("unknown_threshold", 0.15)
        obj._is_known_regime = data.get("is_known_regime", True)
        obj._unknown_count = data.get("unknown_count", 0)
        return obj
