...
| §6.8 Pipeline comparison | All four pipelines | as above | 2025-01-02 → 2026-04-19 | Engine-controlled four-pipeline comparison + figures |
| §6.9 Validation analyses | §6.9.1 Iter 2 pre-flight; §6.9.2 random-search control; §6.9.3 regime-tree feature-set sensitivity | various | 2024 Q2 / 2022 H1 | Three small validation analyses stress-testing load-bearing claims |
...
| Iteration 2 pre-flight (3-month) | 63 × 8 × 8 | 4,032 | Consumer CPU, 9 workers | ≈ 2.8 h | Architectural validation only (§5.6, §6.9.1) |
...
### 6.9 Validation Analyses

The §6.1–6.8 results above stand on three load-bearing claims: that the GA contributes search efficiency beyond uniform random sampling of the same gene-space; that the regime tree underlying the per-leaf island populations is structurally non-degenerate; and that the pre-flight architectural-correctness criterion has discriminating power (does not pass random genomes by chance). Each claim is open to a specific, predictable challenge from a sceptical reader, and we owe that reader an honest answer rather than rhetorical confidence. To that end, three small validation analyses were run after the primary results were in hand and reported below — §6.9.1 covers the pre-flight criterion baseline rate, §6.9.2 covers the random-search control, and §6.9.3 covers the regime-tree feature-set sensitivity. Each is reduced in scope to fit a tractable computational budget; in two cases the corresponding full performance ablation (re-running the GA under an alternate configuration) is explicitly deferred to future work. We report what we found, including limitations, rather than promising a complete answer.

#### 6.9.1 Iteration 2 Architectural Validation (Pre-flight)
