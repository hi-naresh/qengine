...
Beyond the pipeline-level genes, the expanded strategy genome encodes 53 evolved parameters across the 7 tunable groups (after the Filters group and mode-conditional thresholds are excluded; see Section 3.4 Table 2). Of particular interest is the distribution of `signal_mode` values across the trained islands: on the pre-flight training run, 7 distinct signal modes were selected across the 63 islands, including random, ema_cross, rsi, macd, supertrend, and compound variants (ema_rsi, ema_macd, triple). Random entry is retained in some regimes (corresponding to macro-clusters where no directional feature passes the island's evaluation threshold) while other macro-clusters evolve EMA-crossover or other directional signals. Direction bias is likewise varied across islands (long_only, short_only, both). The evolved `sizing_factor` range is [1.5, 2.5] (bounded by the mathematical viability constraint of Section 3.4); evolved `max_levels` ranges [2, 6] across islands; evolved `tp_value` and `hedge_value` span the configured ranges. Although per-regime signal-mode and direction-bias diversity is present in the evolved population, the honest mechanism analysis in Section 6.6 finds that this diversity does not translate into improved Level-0 win rate on the OOS window. The load-bearing controls for the observed drawdown collapse are therefore the depth-related strategy genes (`max_levels`, `sizing_factor`, `hedge_value`) and the pipeline-level genes (`gate_confidence_min`, `abort_aggressiveness`, `recovery_aggression`).
