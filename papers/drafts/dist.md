...
|---|---|---|---|
| Training | 2022-01-01 to 2023-12-31 | 24 months | Island evolution (150 real-engine backtests) |
| In-sample check | 2023-01-01 to 2023-12-31 | 12 months | Sanity check on training period |
| Out-of-sample H1 | 2024-01-01 to 2024-06-30 | 6 months | Primary OOS evaluation |
| Out-of-sample H2 | 2024-07-01 to 2024-12-31 | 6 months | Secondary OOS evaluation |
| Out-of-sample Full | 2024-01-01 to 2024-12-31 | 12 months | Aggregate OOS evaluation (main IslandPilot result) |
| Extended OOS | 2025-01-01 to 2026-04-01 | 15 months | Multi-system comparison on unseen post-training data |

The pipeline (feature selection, regime discovery, island evolution) is trained exclusively on the 2022-2023 period. All 2024 and 2025-2026 results are strictly out-of-sample: no re-fitting of the regime tree or island populations is performed on evaluation data.

The 24-month training window is motivated by two empirical constraints. First, the regime discovery requires a sufficient sample of market conditions: the BIC search for macro-clusters explores k = 2-10 components, and stable GMM estimation of 10 components with full covariance matrices requires at least 10× the number of observations as degrees of freedom (McLachlan & Peel, 2000). For the 5-dimensional macro feature space, each component has 5 + 5×6/2 = 20 covariance parameters, giving a minimum of 200 observations per component, or 2,000 total — well satisfied by 24 months of 30m candles (~17,520 bars after weekend gaps). Second, 24 months captures two complete calendar-year cycles, reducing the risk that the training period coincides with an anomalous market environment. Extending training beyond 24 months (to 36 or 48 months) would push the training window back to 2018-2021, a period that included COVID-19 volatility spikes structurally unlike the post-pandemic normalization in 2022-2023; restricting training to 2022-2023 avoids contamination with those structural breaks.
...
### 5.5 Spread Model and Transaction Cost Assumptions
...
### 6.7 Extended Out-of-Sample Evaluation and Multi-System Comparison (2025-2026)

Table 8 reports performance of all four systems on the extended 15-month out-of-sample period (January 2025 to April 2026), using the same cost model (2-pip OANDA spread), starting balance ($10,000), and strategy preset (original) as the primary evaluation. All results were computed by running the full qengine backtest engine on EUR-USD 30-minute candles with `cost_model=True`. The evaluation period was chosen to extend the OOS window as far as currently available data permits (approximately 15 months beyond the 2024 evaluation), testing all systems on market conditions that are structurally distinct from the 2022-2023 training window.

*Table 8: Extended OOS comparison — all systems on 2025-01-01 to 2026-04-01.*

| Variant | Sessions | Win% | Busts | Bust Rate | PF | Net% | MaxDD% | Signal Blocked% |
|---|---|---|---|---|---|---|---|---|
| Baseline | 1,439 | 84.2% | 228 | 15.8% | 0.782 | -79.6% | -79.7% | — |
| **IslandPilot** | **1,438** | **83.4%** | **238** | **16.6%** | **0.742** | **-83.0%** | **-83.1%** | **0.1%** |
| GTSBotPilot | 605 | 84.0% | 84 | 13.9% | 0.769 | -47.0% | -48.4% | 58.0% |
| FinRLPilot | 1,086 | 55.1% | 211 | 19.4% | 0.762 | -40.7% | -40.7% | 24.5% |

The extended OOS period reveals three distinct findings that were not apparent from the 2024 evaluation.

**Finding 1: IslandPilot shows negative transfer on 2025-2026 data.** The pipeline underperforms its own baseline (PF 0.742 vs 0.782; net loss worsens by 3.4 percentage points; 10 additional busts). The regime tree — trained on 2022-2023 data — classifies approximately 97% of the extended OOS observations into a single leaf (leaf 36, which occupies 89% of candles in the evaluation period). The evolved parameters for leaf 36 were calibrated to 2022-2023 conditions and prove maladaptive under 2025 market dynamics. In 2025, EUR-USD experienced sustained directional moves driven by US tariff policy announcements, producing bust rates (15.8% on the baseline) that exceed the 9.7% observed in 2024. The genome for leaf 36 does not adapt to this higher-bust-rate environment because it was evolved on a training window where the market exhibited different regime structure. This finding directly confirms the degradation pattern identified in Section 6.6: temporal distance from the training window causes progressive deterioration, with the 2025-2026 period (2-3 years post-training) showing complete loss of the 2024 advantage and a reversal to below-baseline performance.

**Finding 2: GTSBotPilot achieves the best net return through session-count reduction.** The trend filter blocks 58% of entry signals (834 of 1,439 potential sessions), reducing total sessions from 1,439 to 605. This reduction is not arbitrary: the EMA derivative filter targets the structural condition most responsible for busts, namely directional momentum. When d1 (first derivative) and d2 (second derivative) of the smoothed price agree in direction, a trend is present; Martingale strategies that enter long against an established downtrend are statistically more likely to reach maximum depth. The filtering produces a bust rate of 13.9% (84 busts / 605 sessions), compared to the baseline's 15.8%. The net return improvement is substantial: -47.0% versus -79.6%, a 32.6 percentage point improvement. However, this benefit accrues entirely from reduced exposure rather than from superior per-trade economics: the profit factor of GTSBotPilot (0.769) is below the baseline (0.782), meaning the individual sessions that DO execute perform worse than baseline sessions, likely because the trend filter preferentially allows entries during the initial (ambiguous) phase of a trend, which is exactly when a Martingale session is most vulnerable. The spread cost per session (approximately 2-14 pips depending on depth) remains constant, but with 58% fewer sessions, the aggregate spread drain is proportionally reduced.

**Finding 3: FinRLPilot achieves the lowest loss through preset selection but with severely impaired win rate.** The tabular Q-learner selects among four discrete presets, achieving a win rate of 55.1% — substantially below the baseline's 84.2%. This anomaly is explained by the policy's preset selections under the tabular state discretization: the 81-state space (3^4 = 81 from 4 features × 3 bins) provides coarse state representation, and the Q-values learned from 2022-2023 training do not generalize accurately to 2025 market conditions. The policy frequently selects the "tight-TP" preset (12-pip take-profit against a 10-pip hedge distance), which in trending conditions causes sessions to close with partial losses before the hedge chain can recover. Nevertheless, the -40.7% net return is the best absolute performance among all tested systems, due to a combination of 24.5% fewer sessions (reducing spread exposure) and the occasional selection of the "conservative" preset (15-pip hedge, 25-pip TP, sqrt sizing, max 4 levels) that limits maximum depth on high-bust sessions. The FinRLPilot result illustrates an important trade-off: a lower win rate can be accompanied by lower absolute losses when the policy successfully prevents the deepest (most expensive) bust sequences.

**Market regime interpretation.** The 2025-2026 period represents materially different market conditions than the 2022-2023 training window. EUR-USD experienced a sustained downtrend from 1.0500 to approximately 1.0300 in early 2025 (USD strength from tariff announcements and Federal Reserve hawkishness), followed by a rebound above 1.0800 by April 2026 (US growth concerns and ECB rate convergence). These large directional moves produce sustained trends rather than the mean-reverting range-bound behaviour that Martingale strategies exploit. The baseline bust rate of 15.8% in 2025-2026 versus 9.7% in 2024 confirms this structural shift: each percentage point increase in bust rate corresponds to approximately a 5-7 percentage point worsening of net annual return given the 35-to-1 win-to-bust loss ratio at maximum depth. IslandPilot, GTSBotPilot, and FinRLPilot all face the same adverse structural conditions; the key differentiator is whether the system reduces exposure (GTSBotPilot, FinRLPilot) or adapts parameters (IslandPilot). Under 2025 conditions, exposure reduction proves more effective than parameter adaptation, primarily because the evolved parameters do not account for the regime shift that occurred two years after training.
...
**Comparison with GTSBotPilot (Rundo et al., 2019).** The GTSBot system uses three coordinated layers: a regression network for trend detection, a Grid System Manager (GSM) enforcing minimum time and price distance between trades, and a Basket Equity System Manager (BESM) monitoring cumulative position profit. Applied to the same EUR-USD Martingale strategy under the same 2-pip spread model, GTSBotPilot achieves net -47.0% versus baseline -79.6% over the extended 2025-2026 OOS period (Table 8), a 32.6 percentage point improvement. The mechanism is session-count reduction: the EMA trend filter blocks 58.0% of entry signals, limiting trading to approximately 605 sessions compared to the baseline's 1,439. IslandPilot, by contrast, blocks only 0.1% of signals (approximately 1 session), relying instead on parameter adaptation rather than entry gating. On the 2024 OOS period (Table 3), IslandPilot achieves a larger profit factor improvement than either GTSBotPilot or FinRLPilot (PF +0.015, or +1.7%), while on the 2025-2026 OOS period GTSBotPilot's session reduction strategy proves more effective at limiting aggregate losses. The fundamental distinction is strategic: IslandPilot adapts HOW the strategy trades (parameter configuration), while GTSBotPilot adapts WHETHER the strategy trades (entry gating). Neither approach is universally dominant; the preferred approach depends on whether the current market regime rewards activity (IslandPilot is more active) or penalises it (GTSBotPilot reduces exposure).

**Comparison with FinRLPilot (Liu et al., 2020).** The FinRL framework applies deep reinforcement learning, specifically PPO, to trading parameter selection. Our FinRLPilot implementation selects among four discrete parameter presets (conservative, moderate, aggressive, tight-TP) using a tabular Q-learner trained on 2022-2023 data. On 2025-2026, FinRLPilot achieves net -40.7% — the lowest absolute loss among all systems — while exhibiting a win rate of only 55.1%, far below the baseline's 84.2%. The low win rate reflects a deterioration in the Q-learner's preset-selection accuracy under 2025 market conditions that differ structurally from the 2022-2023 training environment: the coarse 81-state tabular representation does not generalise the learned action preferences to new market configurations. Nevertheless, FinRLPilot's 24.5% session reduction (1,086 vs 1,439) limits aggregate spread exposure, and the occasional selection of the conservative preset (4 levels, 15-pip hedge, sqrt sizing) avoids the deepest bust sequences. Compared with IslandPilot, FinRLPilot uses a simpler regime representation (four discrete presets vs per-leaf continuous genome), has lower computational training cost (tabular Q-learning vs real-engine genetic evolution), and exhibits worse on the 2024 OOS period (where the trained Q-values are more accurate) but better on the 2025-2026 extended OOS period (because its conservative preset selection mechanically limits bust depth regardless of market regime classification accuracy).
...
The pipeline produces a larger improvement in-sample (2023: net loss reduced by 5.2pp, drawdown by 4.3pp, PF +0.013) than on the primary OOS period (2024 Full: net loss reduced by 1.7pp, drawdown by 1.7pp, PF +0.015), and degrades to below-baseline performance on the extended OOS period (2025-2026: net loss worsens by 3.4pp, PF -0.040). This three-stage progression — IS gain → modest OOS gain → extended OOS loss — follows the pattern documented by Bailey et al. (2014) and is consistent with the temporal decay observed within the 2024 OOS period (H1: +2.4pp net vs H2: +0.4pp net).

The complete picture across all periods quantifies this degradation precisely:

*Table 9: Net return improvement of IslandPilot over baseline across evaluation periods.*

| Period | Months from Training End | IslandPilot Net% | Baseline Net% | Delta |
|---|---|---|---|---|
| 2023 (IS) | 0 | -29.4% | -34.6% | **+5.2pp** |
| 2024 H1 (OOS) | 0-6 | -22.9% | -25.3% | **+2.4pp** |
| 2024 H2 (OOS) | 6-12 | -28.0% | -28.4% | **+0.4pp** |
| 2024 Full (OOS) | 0-12 | -30.5% | -32.2% | **+1.7pp** |
| 2025-2026 (Extended OOS) | 12-27 | -83.0% | -79.6% | **-3.4pp** |

The temporal distance from training end fully explains the progression: at 0 months (in-sample), the evolved regime-to-parameter mappings are most accurate; at 12 months (2024 OOS), some mapping accuracy persists; at 12-27 months (2025-2026), the mappings are sufficiently stale that they are counterproductive. The threshold between positive and negative transfer falls between 12 and 15 months post-training, suggesting that quarterly retraining (every 3 months) would maintain the positive-transfer regime that IslandPilot exhibits in 2024 H1 and H2.

What matters for practical deployment is that the OOS improvement remains positive for the first 12 months post-training. A system that loses its entire in-sample edge immediately out-of-sample has overfit entirely; a system that retains a fraction for the first year has learned transferable structure. The observed 33% retention rate (1.7pp OOS vs 5.2pp IS) over the first 12 months, followed by complete reversal over 12-27 months, establishes the retraining cadence that would maintain the pipeline's advantage: updating the island populations annually (or preferably quarterly) would keep the training window within the positive-transfer horizon.
...
Evaluated on EUR-USD 30-minute data with the OANDA CFD execution model (2-pip spread), the system achieves consistent out-of-sample improvement over the primary evaluation window: profit factor rises from 0.870 to 0.885 on 2024 data (608 sessions), with net loss reduced by 1.7 percentage points, maximum drawdown reduced by 1.7 percentage points, and one fewer bust. The pipeline maintains comparable session throughput while blocking only 5.5% of entry signals. In-sample improvement is larger (PF +0.013, net loss -5.2pp, 5 fewer busts on 877 sessions), indicating modest overfitting that does not eliminate the OOS benefit.

An extended 15-month evaluation (2025-2026) against two pipeline benchmarks reveals a temporal decay dynamic and the limits of regime-aware parameter adaptation under training-distribution shift. GTSBotPilot, a rule-based trend filter adapted from Rundo et al. (2019), achieves -47.0% net (32.6 percentage points above the -79.6% baseline) purely through 58% session-count reduction: by blocking entries during measured directional momentum, the system reduces aggregate spread exposure without any offline training. FinRLPilot, adapted from Liu et al. (2020), achieves -40.7% net — the best absolute performance on the extended period — through conservative preset selection that limits maximum depth on high-bust sessions, despite a win rate that degrades to 55.1% as its Q-values generalise poorly to 2025 market structure. IslandPilot regresses to -83.0%, 3.4 percentage points below the unenhanced baseline, because the regime-to-parameter mappings evolved on 2022-2023 data are maladaptive under the sustained EUR-USD directional moves of 2025 (driven by US tariff policy). Across all evaluation periods, the pipeline shows 33% retention of in-sample gains through the first 12 months OOS, followed by complete reversal between 12 and 27 months. This temporal profile establishes a recommended quarterly retraining cadence: updating island populations every three months would maintain the mappings within the positive-transfer horizon that IslandPilot demonstrates in 2024.

Three conclusions follow from this combined evidence. First, regime-aware parameter adaptation provides consistent short-term benefit (IS and first-year OOS) but requires periodic retraining to remain valid — a constraint shared by any data-driven adaptive system. Second, the competitive advantage of parameter adaptation over rule-based entry filtering depends on market dynamics: IslandPilot's active strategy (0.1% signal blocking) outperforms GTSBotPilot's passive strategy (58% signal blocking) when markets exhibit mean-reverting structure similar to the training window, but the advantage reverses under persistent trending conditions. Third, the minimal evolutionary budget of the current implementation (3 generations, 5 individuals per island) produces modest absolute improvements; the architectural potential is likely larger under a more thorough evolutionary budget made practical through parallelised real-engine evaluation.
